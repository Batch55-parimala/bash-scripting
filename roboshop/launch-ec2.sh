...
PRIVATEIP=$(aws ec2 run-instances --image-id "ami-0f75a13ad2e340a58" --instance-type t3.micro  --security-group-ids sg-0cc65d7ddd0b64dad --tag-specifications "ResourceType=instance,Tags=[{Key=Name,Value=lab}]" | jq '.Instances[].PrivateIpAddress')
